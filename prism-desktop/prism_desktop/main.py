...
class PrismDesktop(SidebarMixin, ChatMixin, TerminalMixin, SettingsMixin, SystemMixin, MCPMixin, BrowserMixin):
    def __init__(self, page: ft.Page):
        ...
        self.review_enabled_switch = ft.Switch(label="后台自我复盘", value=bool(int(os.getenv("PRISM_REVIEW_ENABLED", "1") or 1)), width=200, label_style=ft.TextStyle(size=12))
        self.review_interval_field = ft.TextField(label="复盘间隔(轮数)", value=str(int(os.getenv("PRISM_REVIEW_INTERVAL", "5") or 5)), width=200, keyboard_type=ft.KeyboardType.NUMBER, dense=True)
